...
```text
Planner:  join(room="design-review", name="plan")  →  if waiter is missing, start command using how
Executor: join(room="design-review", name="exec")  →  if waiter is missing, start command using how

Planner: send(text="Please check this design.", to="exec")
Planner: send(text="Status request for everyone.")  →  broadcast
```

- `join(room, name)` fixes the room and sender identity for the connection, then returns the participant ID, registered role names, `waiter` state, `command`, `how`, and next-call instructions. Call it on every new MCP connection, including after a client or server restart. If `waiter` is `missing`, start `command` using `how`; if it is `active`, do nothing. Repeat the same call only to refresh participants or waiter state. Registration alone does not indicate online presence.
...
Run one waiter per participant and reuse it after handling messages. A participant lock rejects a second waiter process. Do not poll. Normal MCP shutdown invalidates the waiter. After a crash or restart, reconnect and call `join()` again; it invalidates the previous waiter's session token, recovers pending messages, and returns the new command.
...
Messages are stored individually and delivered in order. A message is acknowledged only after stdout is successfully flushed or `codex queue` succeeds. Failed delivery leaves the message pending. Failed Codex queue submissions retry after 5 seconds; stdout failures stop the waiter. Fix the output error, call `join()` to confirm `waiter` is `missing`, and run its returned command to resume.
